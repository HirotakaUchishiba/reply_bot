...
def get_recent_replies(limit: int = 5) -> List[Dict[str, Any]]:
    ...
    response = table.scan(
        ...
        ProjectionExpression="context_id, subject, body_redacted, "
...
def get_similar_replies(
    ...
    # Remove common words
    stop_words = {"の", "は", "が", "を", "に", "で", "と", "から", "まで", "について", "です", "ます", "です", "ます", "お", "ご", "いただき", "ありがとう"}
    ...
    for item in items:
        ...
        if current_keywords and item_keywords:
            similarity = len(current_keywords & item_keywords) / len(current_keywords | item_keywords)
            if similarity > 0.1:  # Minimum similarity threshold
                scored_items.append((item, similarity))
    
    # Sort by similarity score and timestamp
    scored_items.sort(key=lambda x: (x[1], x[0].get("replied_at", 0)), reverse=True)
