...
def _assess_completeness(reply: str, inquiry: str, subject: str, issues: List[str]) -> float:
    """Assess if the reply addresses the inquiry completely."""
    score = 0.0
    
    # Check if reply acknowledges the inquiry
    if "お問い合わせ" in reply or "ご質問" in reply or "ご連絡" in reply:
        score += 0.3
    else:
    ...
    contact_indicators = ["お電話", "メール", "ご連絡", "お問い合わせ"]
