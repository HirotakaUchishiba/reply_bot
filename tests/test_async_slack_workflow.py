...
class TestAsyncSlackWorkflow:
    ...
    def test_async_generation_workflow_with_endpoint(self):
        """Test complete async workflow when ASYNC_GENERATION_ENDPOINT is configured"""
        # Mock configuration with async endpoint
        mock_config = MagicMock()
        mock_config.async_generation_endpoint = "https://test-cloudrun.example.com/async/generate"
        mock_config.async_generation_auth_header = "Bearer test-token"
        mock_config.stage = "staging"
        mock_config.slack_signing_secret_arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:test"
        mock_config.slack_app_secret_arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:test"
        ...
        with (
            ...
            patch("src.app.router.resolve_slack_credentials", return_value=mock_creds),
            patch("src.app.router.verify_slack_signature", return_value=True),
            patch("src.app.router.get_context_item", return_value=mock_context),
            ...
            assert request.get_full_url() == "https://test-cloudrun.example.com/async/generate"
    ...
    def test_sync_generation_workflow_without_endpoint(self):
        """Test sync workflow when ASYNC_GENERATION_ENDPOINT is not configured"""
        ...
        with (
            ...
            patch("src.app.router.verify_slack_signature", return_value=True),
            patch("src.app.router.get_context_item", return_value=mock_context),
            patch("src.app.router.generate_reply_draft", return_value="Generated reply text"),
            patch("src.app.router.reidentify", return_value="Hello John Doe, thank you for your message."),
            patch("src.app.router.SlackClient") as mock_slack_client,
            patch("urllib.request.urlopen") as mock_urlopen
        ):
            # Mock Slack client
            ...
            assert "Hello John Doe, thank you for your message." in initial_text
    ...
    def test_async_endpoint_failure_handling(self):
        ...
        with (
            ...
            # Note: Error handling is done silently in the current implementation
            # The async endpoint failure doesn't prevent modal opening
