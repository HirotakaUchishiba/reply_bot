...
with patch.dict('sys.modules', {
    ...
    from src.lambda.common.pii import redact_and_map, reidentify
...
class TestPIIRedaction:
    ...
    def test_redact_and_map_email(self):
        ...
        with patch('src.lambda.common.pii.analyzer') as mock_analyzer, \
             patch('src.lambda.common.pii.anonymizer') as mock_anonymizer:
            
            # Mock analyzer results
            mock_analyzer.analyze.return_value = [
                MagicMock(entity_type="EMAIL_ADDRESS", start=5, end=20, score=0.9)
            ]
            
            # Mock anonymizer results
            mock_anonymizer.anonymize.return_value = MagicMock(
                text="連絡先は [EMAIL_1] です",
                items=[MagicMock(operator="replace", start=5, end=20, text="[EMAIL_1]")]
            )
    ...
    def test_redact_and_map_phone(self):
        ...
        with patch('src.lambda.common.pii.analyzer') as mock_analyzer, \
            ...
            # Mock analyzer results
            mock_analyzer.analyze.return_value = [
                MagicMock(entity_type="PHONE_NUMBER", start=5, end=18, score=0.9)
            ]
            
            # Mock anonymizer results
            mock_anonymizer.anonymize.return_value = MagicMock(
                text="電話番号は [PHONE_1] です",
                items=[MagicMock(operator="replace", start=5, end=18, text="[PHONE_1]")]
            )
